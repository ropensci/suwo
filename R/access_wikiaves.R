...
#'
#' @description
#' WikiAves (\url{https://www.wikiaves.com.br}) sits behind Cloudflare's bot
#' protection, which blocks plain \code{httr2} requests with an HTTP 403 and
#' a "Just a moment..." challenge page. This function works around that by
...
#'
#' By default (\code{set_env = TRUE}) this string is also stored directly in
#' the \code{wikiaves_cookies} environment variable for the current R
#' session, which is what \code{\link{query_wikiaves}} reads from by
#' default -- so in the common case, calling \code{access_wikiaves()} once
#' is enough to authenticate every subsequent \code{query_wikiaves()} call
#' in the session without passing \code{cookies} explicitly. See
#' \strong{Details}.
#'
...
#' temporary directory if \code{chrome_bin} and \code{user_data_dir} are
#' left at their default (\code{NULL}); see \strong{Details}.
#'
#' @param chrome_bin Character. Name or path of the browser executable to
#'   launch. Default \code{NULL}, which triggers auto-detection of a
#'   Chrome- or Chromium-based browser for the current operating system
#'   (see \strong{Details}). Set this explicitly to override
#'   auto-detection or to point at a non-standard install location, e.g.
#'   \code{access_wikiaves("chromium")} on Linux,
#'   \code{"/Applications/Chromium.app/Contents/MacOS/Chromium"} on macOS,
#'   or \code{"C:/Program Files/Google/Chrome/Application/chrome.exe"} on
#'   Windows. Messages printed while the function runs refer to whichever
#'   browser \code{chrome_bin} resolves to, so it is easy to tell which one
#'   is in use.
#' @param port Integer. The TCP port used for the browser's DevTools remote
#'   debugging protocol. Default \code{9333}. If a browser instance is
#'   already listening on this port, it is reused instead of launching a
#'   new one.
#' @param url Character. The WikiAves URL to navigate to and extract cookies
#'   from. Default \code{"https://www.wikiaves.com.br/"}. Changing this is
#'   only useful for testing against a different page on the same domain.
#' @param timeout Numeric. Maximum time, in seconds, to wait for a
#'   single round-trip response (cookies, user agent, and page title) over
#'   the DevTools websocket connection. Default \code{10}.
...
#'   Default \code{15}. Only relevant when the browser is not already
#'   running on \code{port}.
#' @param challenge_wait Numeric. Maximum total time, in seconds, to
#'   keep polling the page while Cloudflare's "Just a moment..." challenge
#'   is showing. Default \code{30}. If the challenge has not cleared within
#'   this window the function stops with an informative error; the person
#'   can solve the challenge manually in the visible browser window (e.g.
#'   clicking a checkbox) while this function is polling, and it will pick
#'   up the cleared page on the next check.
#' @param user_data_dir Character. Filesystem path to the browser's user
#'   data directory used for this session. Default \code{NULL}, which
#'   resolves to a \code{"chrome-wikiaves-profile"} folder inside
#'   \code{tempdir()} -- a location valid on Linux, macOS, and Windows
#'   alike. Reusing the same directory across calls lets the browser and
#'   Cloudflare recognize the profile as previously trusted, which often
#'   lets later calls clear the challenge automatically without manual
#'   interaction.
#' @param set_env Logical. If \code{TRUE} (the default), also stores the
#'   returned cookies string in the \code{wikiaves_cookies} environment
#'   variable for the current R session via \code{Sys.setenv()}, so that
#'   \code{\link{query_wikiaves}}'s default \code{cookies} argument picks
#'   it up automatically without needing to pass it explicitly. Set to
#'   \code{FALSE} to make the function a pure getter that only returns the
#'   cookies string without modifying the session's environment
#'   variables -- useful if you want to manage the credentials yourself
#'   (e.g. pass them directly into a single \code{query_wikiaves()} call,
#'   store them somewhere else, or use a different session's variable).
#'
#' @details
#' This function requires the \pkg{websocket} and \pkg{later} packages,
#' which are listed under \code{Suggests} rather than \code{Imports} since
#' this workflow is only needed for the WikiAves data source and depends on
#' a local Chrome- or Chromium-based browser installation and a usable
#' display (e.g. the \code{DISPLAY} environment variable on Linux).
#'
#' \strong{Browser auto-detection.} When \code{chrome_bin = NULL} (the
#' default), the function first checks whether the \pkg{chromote} package
#' happens to be installed and, if so, uses its exported
#' \code{\link[chromote]{find_chrome}} to locate a browser -- this respects
#' the \code{CHROMOTE_CHROME} environment variable and \pkg{chromote}'s own
#' per-OS search logic. \pkg{chromote} is not a dependency of this
#' function; if it is not installed, or its detection does not find
#' anything, the function falls back to its own candidate-path search:
#' \itemize{
#'   \item \strong{Linux}: \code{google-chrome}, \code{google-chrome-stable},
#'     \code{chromium}, and \code{chromium-browser}, in that order, checked
#'     via \code{Sys.which()}.
#'   \item \strong{macOS}: \code{/Applications/Google Chrome.app/Contents/MacOS/Google Chrome}
#'     and \code{/Applications/Chromium.app/Contents/MacOS/Chromium}.
#'   \item \strong{Windows}: the \code{chrome.exe} install locations under
#'     \code{Program Files} and \code{Program Files (x86)}, as well as a
#'     per-user AppData install location.
#' }
#' If none of these are found, the function stops with an error explaining
#' how to set \code{chrome_bin} manually. Auto-detected paths have only
#' been tested against typical default install locations; if your browser
#' is installed somewhere non-standard, pass its path explicitly via
#' \code{chrome_bin}.
#'
#' On first use (or once the session has expired, which typically
#' happens within a couple of hours), Cloudflare may present a visible
#' interactive challenge in the launched browser window. Switching to that
#' window and completing the challenge is a normal part of the workflow;
#' this function polls for up to \code{challenge_wait} seconds so the
#' person has time to do so before it gives up, and prints a message once
#' the cookies have been successfully retrieved.
#'
#' \strong{Automatic environment variable.} By default
#' (\code{set_env = TRUE}), the returned cookies string is also written to
#' \code{Sys.setenv(wikiaves_cookies = ...)} before the function returns.
#' Since \code{\link{query_wikiaves}}'s \code{cookies} argument defaults to
#' \code{Sys.getenv("wikiaves_cookies")}, this means a bare
#' \code{access_wikiaves()} call is normally all that is needed to
#' authenticate every \code{query_wikiaves()} call made afterwards in the
#' same R session -- there is no need to manually capture and pass the
#' return value unless \code{set_env = FALSE} is used.
#'
#' @return
#' A single JSON-encoded character string containing \code{cf_clearance},
#' \code{PHPSESSID}, and \code{browser_ua}. This is the format expected by
#' the \code{cookies} argument of \code{\link{query_wikiaves}}. The string
